...
export default function NuevoExpediente() {
  ...
  async function handleSubmit() {
    ...
    try {
      await addExpediente({
        client: client.trim(), clientPhone: clientPhone.trim(), clientEmail: clientEmail.trim(),
        type, status, lawyer: lawyer.trim(), court: court.trim(),
        counterpart: counterpart.trim(), description: description.trim(),
    ...
    <div className="flex h-full bg-[var(--background)]">
      ...
      <main className="flex flex-col flex-1 gap-6 p-8 overflow-auto">
        ...
        <div className="flex flex-col flex-1 bg-[var(--card)] border border-[var(--border)] shadow-sm p-6 gap-6 overflow-auto">
          ...
          <div className="flex gap-4 w-full">
            <SelectGroup label="Tipo de Caso *" value={type} options={["Divorcio", "Civil", "Penal", "Laboral", "Mercantil", "Familiar", "Contencioso"]} onChange={setType} className="flex-1" />
          ...
          <InputGroup label="Descripcion del Caso" placeholder="Detalle brevemente el caso..." value={description} onChange={setDescription} className="w-full" />
          ...
          {error && <p className="font-secondary text-sm text-[var(--destructive)]">{error}</p>}
          <div className="flex-1" />
          <div className="flex justify-end gap-3 w-full">
            <Button variant="outline" onClick={() => router.push("/expedientes/lista")}>Cancelar</Button>
            <Button icon="save" onClick={handleSubmit} className={saving ? "opacity-60 pointer-events-none" : ""}>
